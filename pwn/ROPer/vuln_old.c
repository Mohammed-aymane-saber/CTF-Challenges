...
/* ── ROP gadgets ─────────────────────────────────────────────────────────── */
/* Embedded in .text.gadgets so the linker never strips them.                */
/* solve.py finds them by raw byte scan — independent of pwntools ROP cache. */

/* gadget 1 — pop rdi ; ret   bytes: 5f c3 */
__attribute__((used, noinline, section(".text.gadgets")))
void __pop_rdi_gadget(void) {
    __asm__ volatile ("pop %rdi\n\t" "ret\n\t");
}

/* gadget 2 — mov rdi, rax ; ret   bytes: 48 89 c7 c3 */
__attribute__((used, noinline, section(".text.gadgets")))
void __mov_rdi_rax_gadget(void) {
